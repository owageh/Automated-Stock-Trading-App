...
for symbol in symbols:
    minute_bars = api.get_barset(
        symbol, 'minute', start=current_date, end=current_date).df
    # print(minute_bars)
...
#    print(after_opening_range_bars)
    ...
    if not after_opening_range_breakdown.empty:
        if symbol not in existing_order_symbols:
            ...
            print(
                f"Placing Sell short order for {symbol} at {limit_price}, closed_below {opening_range_low} at {after_opening_range_breakdown.iloc[0]}")
            ...
            try:
                ...
                messages.append(
                    f"Placing Sell short order for {symbol} at {limit_price}, closed_below {opening_range_low}\n\n{after_opening_range_breakdown.iloc[0]}\n\n")
